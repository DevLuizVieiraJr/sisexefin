...
Este documento descreve as regras de negócio da **Aba Dados Básicos** (painel 0) do módulo de Títulos de Crédito (TC), alinhado ao modelo de [Regras de Negócio - Aba Liquidação.md](./Regras%20de%20Negócio%20-%20Aba%20Liquidação.md).
...
| Tipo de TC  | Select | Sim         | NFE, NFSE, FAT, BO, OUT — filtra NE na busca.   |
...
| RC do contrato     | Select | Conforme UI | Depende do contrato. |
...
| Liquidação    | Indireta: NP e LFs atuam sobre `empenhosVinculados` definidos após Processamento. |
| Financeiro    | OP e PF completam dados após liquidação. |
...
| `fornecedor`, `instrumento`, `rc`, dados de vigência/fiscal | Contrato. |
...
Quando existir **edição em curso** noutros painéis do mesmo formulário (ex.: painel de complementação de NE na aba Processamento), a navegação para outra aba ou o fecho do TC deve seguir as regras documentadas em [Regras de Negócio - Aba Processamento.md](./Regras%20de%20Negócio%20-%20Aba%20Processamento.md) (secção sobre painel NE e navegação).
