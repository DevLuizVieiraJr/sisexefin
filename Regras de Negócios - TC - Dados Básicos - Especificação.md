...
### Fonte oficial da interface TC

- O CRUD de Título de Crédito é realizado **exclusivamente** em `titulos.html` com lógica em `script-titulos-spa.js`.
- A seção antiga de TC dentro de `sistema.html` foi desativada para evitar divergência de regras e inconsistência de dados.
...
| 3 | Tipo de TC | select | NFE, NFSE, FAT, BO, NFG | Conforme `titulos.html` | Sim | Sim |
...
| 13 | RC do Contrato | select | Um RC (contrato pode ter 1 ou mais RC) | Vazio | **Sim se o contrato tiver RC cadastrado; senão não obrigatório** | Não |
...
| ug | string | 741000 |
| tipoTC | string | NFE, NFSE, FAT, BO ou NFG (NF Genérica) |
...
| fornecedorNome | string | Nome do fornecedor (preferencial) |
| fornecedorCnpj | string | CNPJ do fornecedor (preferencial) |
| fornecedor | string | Legado: texto único; evitar em novos fluxos |
...
- UG: sempre 741000
- Tipo de TC: uma das opções NFE, NFSE, FAT, BO, NFG
...
- RC: obrigatório **somente se** o contrato possuir RC cadastrado; se não houver RC no contrato, pode salvar sem RC
