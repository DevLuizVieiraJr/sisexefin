...
**Recomendação:** criar uma **coleção própria** para a pré-liquidação / lote (nome sugerido: `preLiquidacoes` ou `liquidacoesBloco`), em vez de armazenar apenas em campos espalhados nos TCs.
...
*(Campos exatos ficam para definição na implementação, alinhados às respostas da seção 8.)*
...
5. **Status do lote** (e espelho nos TCs, se adotado):
   - **Para pagamento parcial**: nem todas as NEs consolidadas do lote têm LF associada, **ou** nem todas as LF associadas já têm PF (conforme `lfpf` / regra de “LF atendida”).
   - **Para pagamento**: todas as NEs do lote com LF definida **e** todas essas LF com PF satisfeita (alinhado à ideia de “todas atendidas”).

*(Nomes exatos dos status no Firestore podem ser `Para Pagamento` / `Para pagamento parcial` ou constantes internas — ver §8.10.)*
...
6. **DetaCustos** – a definir.
7. **Histórico** – a definir.
...
| **Fechado** | NP e data informadas; política de correção via nova versão ou novo lote (não editar silenciosamente). |
| **Cancelado** | Lote invalidado; consulta/auditoria preservada. |
...
### 7.3 Após Fechado – histórico

- **Recomendação:** não sobrescrever o lote fechado com correções estruturais.
- Preferir **versionamento** (versões 1, 2, 3 no mesmo processo) ou **novo lote** referenciando o anterior (`substitui` / `corrige`), preservando o histórico do lote antigo.

### 7.4 Cancelamento

- **Rascunho:** desvincular TCs do lote (limpar referências no TC se já existirem), marcar lote como cancelado.
- **Fechado (com NP):** **não** remover automaticamente NP/data de todos os TCs **sem regra explícita** e confirmação forte (risco de inconsistência com SIAFI e com a coleção `np` / LF).
- Cancelamento deve registrar **motivo**, **usuário** e **data/hora**.

---

## 8. Decisões de produto (perguntas e respostas)

Esta secção consolida as **respostas do utilizador**, as **decisões recomendadas** onde ainda havia dúvida, e as **dúvidas residuais** para implementação (§8.11).
...
5. **DAuLiq: só download local vs persistir?**  
   **Resposta original:** pergunta aberta (“o que sugere?”).

**Decisão recomendada (para fechar o desenho):**

- **Persistir metadados** no documento `preLiquidacoes`: data/hora de geração, utilizador, versão do layout, hash ou número de versão do consolidado usado, nome do ficheiro sugerido.
- **Persistir o ficheiro** em **Firebase Storage** (ou equivalente), com URL no documento do lote — permite reimpressão, auditoria e o mesmo link na aba Liquidação do TC.
- Manter **download imediato** no browser após gerar (UX).
- Se Storage não estiver configurado num ambiente: fallback para só metadados + download local, com aviso na UI.

**Dúvida residual:** confirmar se o projeto já tem bucket Storage e regras de segurança alinhadas ao RBAC (§8.7).
...
**Decisão recomendada:** adotar **(B) correção restrita de NP/data** (motivo obrigatório + auditoria). Alterações estruturais (trocar TCs do carrinho, reabrir consolidação) devem passar por **nova pré-liquidação** ou fluxo de **versionamento** explícito.
...
**Complemento recomendado (cenário não coberto só pela resposta acima):**

| Ação | Efeito sugerido em `np` / `dataLiquidacao` no TC removido ou afetado |
|------|------------------------------------------------------------------------|
| **Remover um TC do lote** (lote continua válido) | Limpar no TC apenas se a NP/data tinham sido propagadas **a partir deste lote**; motivo obrigatório; histórico; rever `arrayUnion` na coleção `np` se necessário (pode exigir política explícita para retirar vínculo). |
| **Cancelar pré-liquidação inteira** (com NP já informada) | **Não** apagar automaticamente NP em todos os TCs; usar fluxo dedicado (perfil elevado, confirmação dupla, motivo) **ou** tratar como exceção operacional documentada. |

**Dúvida residual:** definir se a retirada do TC remove também o `tituloId` do documento `np` no Firestore ou apenas regista desvinculação no histórico (impacto em relatórios).

### 8.7 Permissões (RBAC)

9. **Permissões**  
   **Resposta:** padrão **CRUD**; **eliminação física (delete)** apenas **admin**; **cancelamento** e **inativação** como ações distintas com permissões próprias.

**Sugestão de chaves (a validar no `admin` / Firestore rules):** `preliquidacao_ler`, `preliquidacao_inserir`, `preliquidacao_editar`, `preliquidacao_inativar`, `preliquidacao_cancelar`, `preliquidacao_excluir` (admin), `preliquidacao_gerar_pdf`, `preliquidacao_fechar_np`, `preliquidacao_associar_lf`.

### 8.8 DetaCustos e Histórico (secções 6 e 7 do DAuLiq em PDF)

10. **Conteúdo**  
    **Resposta (DetaCustos / detalhe por NE):** lista das **NE em bloco** (incluindo casos de NE repetida/agregada) com dados por vínculo no TC: **NE**, **subitem/subelemento**, **centro de custos**, **UG beneficiária**, **valor usado**.

**Decisão recomendada para a secção “Histórico” do PDF:** não duplicar todo o `historicoStatus` do TC; incluir **resumo** dos marcos relevantes à pré-liquidação (criação do lote, geração de DAuLiq, fecho NP, associação LF, mudança para Para pagamento / Parcial), com data e utilizador — fonte: array `historico` / eventos gravados em `preLiquidacoes` (e opcionalmente referência ao TC).

**Dúvida residual:** confirmar se o utilizador quer a secção 7 do PDF **idêntica** ao histórico técnico do Firestore ou apenas este resumo operacional.
...
14. **Status Para pagamento / Parcial**  
    **Resposta:** **atualizar todos** os `titulos` do lote (além do documento `preLiquidacoes`).

**Nota:** implementar **atualização em lote** de `titulos.status` e de `empenhosVinculados` quando a pré-liquidação concluir etapas (§9), para não depender de “abrir e salvar” cada TC.

---

### 8.11 Dúvidas residuais (implementação / produto)

Questões que **ainda convém fechar** com o negócio ou com a infraestrutura:

1. **Storage:** bucket Firebase Storage disponível e limite de tamanho por PDF; regra de quem pode ler o ficheiro (mesmo que `titulos_ler`)?
2. **Coleção `np`:** ao remover TC do lote, deve-se **remover** o ID do TC de `titulosVinculados` no documento da NP ou apenas **anotar desvinculação** no histórico (sem alterar o array)?
3. **Status do TC:** quando o lote está “Parcial”, o `titulos.status` deve ser literalmente `Para pagamento parcial` ou manter `Liquidado` / `Aguardando Financeiro` com um **subcampo** `statusPreLiquidacao` (evita conflito com o fluxo atual do TC)?
4. **Dois LF raros:** quem valida no órgão que aquela NE pode ter 2 LF no mesmo lote — necessidade de **anexo** ou campo “motivo” no `itensNe[]`?
5. **Correção de NP:** limite de vezes ou janela de tempo (ex.: só até primeira associação LF)?
6. **Sequência `PL-00000/AAAA`:** geração **atómica** (Cloud Function / transação) para evitar duplicados em uso concorrente?
7. **Histórico no PDF (secção 7):** lista completa de eventos ou só marcos (recomendado marcos)?
8. **Alinhamento com SIAFI:** NP única para o lote — confirmar que **todos** os TCs do carrinho partilham **exatamente** a mesma NP e `dataLiquidacao` após fecho.

---

**Estado do documento:** a secção 8 está **fechada para desenvolvimento inicial**, restando validação das **dúvidas da §8.11** e qualquer ajuste fino após primeiro sprint.
...
*Documento vivo: secção 8 consolidada com respostas, decisões recomendadas e dúvidas residuais (§8.11). Revisar §8.11 antes do go-live.*
